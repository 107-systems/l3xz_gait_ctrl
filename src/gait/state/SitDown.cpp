...
SitDown::SitDown(rclcpp::Logger const logger, rclcpp::Clock::SharedPtr const clock)
: StateBase(logger, clock)
, _citer{SIT_DOWN_TRAJ.cbegin()}
{
...
std::tuple<StateBase *, ControllerOutput> SitDown::update(kinematic::Engine const & engine, ControllerInput const & input, ControllerOutput const & prev_output)
{
  ...
  for (auto leg : LEG_LIST)
  {
    ...
    if (!ik_output.has_value())
    {
      RCLCPP_ERROR(_logger,
                   "StandUp::SitDown, engine.ik_solve failed for (%0.2f, %0.2f, %0.2f / %0.2f, %0.2f, %0.2f)",
                   target_x,
                   target_y,
                   target_z,
                   coxa_deg_actual,
    ...
    {
      RCLCPP_ERROR(_logger,
                   "StandUp::SitDown, engine.fk_solve failed for (%0.2f, %0.2f, %0.2f)",
                   coxa_deg_actual,
                   femur_deg_actual,
                   tibia_deg_actual);
      return {this, next_output};
    ...
    if (target_err > 20.0f)
    {
      RCLCPP_INFO_THROTTLE(_logger,
                           *_clock,
                           1000,
                           "StandUp::SitDown, target = (%0.2f, %0.2f, %0.2f), fk_output = (%0.2f, %0.2f, %0.2f), target_err = %0.2f",
                           target_x,
                           target_y,
                           target_z,
  ...
  if (!all_target_positions_reached)
  {
    RCLCPP_INFO_THROTTLE(_logger,
                         *_clock,
                         1000,
                         "l3xz::gait::state::SitDown::update: target position not reached for [ %s]",
